...
"""PROMPT 20: experimental camera wheel tracker CLI.
    ...
    python wheel_cam.py --source screen --region 100,80,640,640   # grab layar
    python wheel_cam.py --source screen --list-monitors           # daftar monitor
...
def main(argv=None):
    ...
    parser.add_argument("--source", default="0",
                        help="Camera index (e.g. 0), path to a video file, or "
                             "'screen' to capture a region of the screen.")
    parser.add_argument("--region", default=None,
                        help="Screen region 'x,y,w,h' (mode --source screen). "
                             "Kosong = seluruh monitor (--monitor).")
    parser.add_argument("--monitor", type=int, default=1,
                        help="Indeks monitor untuk --source screen (1=utama, "
                             "0=semua layar). Dipakai bila --region kosong.")
    ...
    parser.add_argument("--center", default=None,
                        help="Pusat roda 'x,y' relatif ke frame/region "
                             "(default: deteksi otomatis).")
    parser.add_argument("--throttle", type=float, default=0.0,
                        help="Jeda detik antar grab layar (batasi CPU/FPS).")
    parser.add_argument("--list-monitors", action="store_true",
                        help="Cetak daftar monitor (mode --source screen) lalu keluar.")
    ...
    screen_mode = str(args.source).strip().lower() == "screen"
    ...
    # Parse shared optional knobs (HSV / center) once, with friendly errors.
    try:
        hsv_ranges = parse_hsv(args.hsv)
        center = parse_center(args.center)
        region = parse_region(args.region) if (screen_mode and args.region) else None
    ...
    # "0" -> webcam index 0; otherwise treat as a file path (camera mode only).
    ...
    for r in range(1, rounds + 1):
        if screen_mode:
            ...
            tracker = CameraWheelTracker(
                source=source, center=center, hsv_ranges=hsv_ranges,
            )
        try:
            result = tracker.run(
